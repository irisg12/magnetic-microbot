...
RUN = false;            % Enables/disables visualization for debugging
vid_bool = true;        % true for video, false for image
kernel = strel('square', 3);  % 3x3 kernel for noise removal

low_orange_thresh = [1/360, 5/255, 95/255];
...
%% Live Video (or Image Processing)
figure(1);
set(gcf, 'Units', 'normalized', 'Position', [0.02, 0.6, 0.45, 0.3]); % sets screen location
if RUN
    figure(2);
    set(gcf, 'Units', 'normalized', 'Position', [0.3, 0.4, 0.2, 0.5]); % sets screen location
end
while true
    if ~vid_bool                                                            % image code
        orig_image = imread("TeamImage1.jpg");
    else
    ...
    field = reducepoly(contours, .05); %.05 is the tolerance factor, adjustable
    % shows the frame
    imshow(orig_image);
    hold on;
...
for i = 1:length(contours)
    ...
    if abs(box(3)+2 - MAGSIZE) < 5.25 && abs(box(4)+2 - MAGSIZE) < 5.25
...
map = map.*-1; % obstacles are -1 squares
